(function($) {    
	function calendarWidget(el, params) { 
		...
		var table = '';
			...
			for (d=0; d<7; d++) {
				table += '<th class="weekday">' + dayNames[d].substring(0,3) + '.</th>';
			}
            ...
            for (j=0;j<42;j++){
              ...
              if ((j<firstDay)){
				...
				if(j-firstDay+1 == day)
					table += ('<td class="current-month day'+(j-firstDay+1)+'" id="curday"><span class="day"><b>'+(j-firstDay+1)+'</b></span></td>');
				else
		...
		$('.nav-prev').click(function(){ console.log("prev") });
		$('.nav-next').click(function(){ console.log("next") });
	}
